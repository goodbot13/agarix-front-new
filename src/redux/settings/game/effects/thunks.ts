...
export const thunkSetCellRemoveAnimation = (
  type: CellRemoveAnimationType,
): GameEffectsThunkActionTypes => (dispatch) => {
  dispatch(setCellRemoveAnimation(type));
  GameEffectsSettings.setCellRemoveAnimation(type);
};

export const thunkSetCellRemoveAnimationForHiddenSpectator = (
  value: boolean,
): GameEffectsThunkActionTypes => (dispatch) => {
...
export const thunkSetSpawnAnimation = (
...
export const thunkSetWtfRgbMode = (wtfRgbMode: boolean): GameEffectsThunkActionTypes => (
  dispatch,
) => {
