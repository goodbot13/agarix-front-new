    ...
    >
      {(totalMass || totalPlayers) && (
        <div className={css.stats}>
          {totalMass && (
            <div className={css.totalMass}>
              <FontAwesomeIcon icon={faCookieBite} style={{ marginRight: 4 }} />
              Mass: {shortMass ? numberToK(totalMassValue) : totalMassValue}
            </div>
          )}
          {totalPlayers && (
            <div className={css.totalPlayers}>
              <FontAwesomeIcon icon={faUsers} style={{ marginRight: 1 }} /> Players:{' '}
              {players.length}
              <FontAwesomeIcon style={{ marginLeft: 8 }} icon={faEye} /> {[displayAmount]}
            </div>
          )}
        </div>
      )}
      <div className={css.players}>
        {players
          ...
          .map((player) => (
            <div className={css.player} key={player.id}>
              {displayPlayerMass && (
                <div className={css.mass}>
                  <FontAwesomeIcon icon={faCookieBite} style={{ marginRight: 2 }} /> [
                  {shortMass ? numberToK(player.mass) : player.mass}]
                </div>
              )}
