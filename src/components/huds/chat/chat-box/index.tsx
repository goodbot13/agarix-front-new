...
const ChatBox: FC<TChatBox> = ({
  ...
  const getBackgroundColor = useCallback(
    (type: TChatMessageType): string => {
      switch (type) {
        case 'GAME':
          return rgbToCssString(gameMessageBackgroundColor);
        case 'COMMAND':
          return rgbToCssString(commandMessageBackgroundColor);
        case 'PLAYER':
          return 'transparent';
      }
    },
  ...
  const getFontColor = useCallback(
    (type: TChatMessageType): string => {
      switch (type) {
        case 'GAME':
          return rgbToCssString(gameMessageFontColor);
        case 'COMMAND':
          return rgbToCssString(commandMessageFontColor);
        case 'PLAYER':
          return rgbToCssString(fontColor);
      }
    },
    [gameMessageFontColor, commandMessageFontColor, fontColor],
  );

  const getBorderBottomColor = useCallback(
    (type: TChatMessageType): string => {
      switch (type) {
        case 'GAME':
